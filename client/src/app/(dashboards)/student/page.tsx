...
import { LogOut, GraduationCap, Calendar, FileText } from 'lucide-react';
    ...
    <ProtectedRoute allowedRoles={['student']}>
      <div className="min-h-screen bg-neutral-950 text-white p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { title: 'Academic Profile', icon: <GraduationCap className="w-8 h-8" />, desc: 'View grades and marks' },
            { title: 'My Schedule', icon: <Calendar className="w-8 h-8" />, desc: 'Timetable and deadlines' },
            { title: 'Study Materials', icon: <FileText className="w-8 h-8" />, desc: 'Access course notes' },
          ].map((item, i) => (
